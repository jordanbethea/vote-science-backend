...
class RankingConstraintTest extends PlaySpec {
    ...
    "Ranked data constraint" must {
      ...
      "Fail with out of range rank in single question result" in {
        val outOfRangeRankData = RankedModelDTO(Seq(Seq(
          RankedChoiceDTO(2L, 41L, 1),
          RankedChoiceDTO(2L, 42L, 2),
          RankedChoiceDTO(2L, 43L, 5)
        )))
        val result: ValidationResult = RankedModelDTO.rankedValuesConstraint(outOfRangeRankData)
        result match{
          case Valid => fail
          case Invalid(errors) => {
            errors.length must be (1)
      ...
      "Fail with duplicate candidate chosen in single question result" in {
        val duplicateQuestionData = RankedModelDTO(Seq(Seq(
          RankedChoiceDTO(2L, 41L, 1),
          RankedChoiceDTO(2L, 41L, 2),
          RankedChoiceDTO(2L, 43L, 3)
        )))
        val result: ValidationResult = RankedModelDTO.rankedValuesConstraint(duplicateQuestionData)
        result match{
          case Valid => fail
          case Invalid(errors) => {
            errors.length must be (1)
